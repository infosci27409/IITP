...
def compute_metric(known, novel):
    ...
    tnr_at_tpr95[stype] = fp[stype][tpr95_pos] / num_n
    mtypes = ['FPR', 'AUROC']
    results = dict()
    results[stype] = dict()
...
def get_curve(dir_name, stype = 'PoT'):
    tp, fp = dict(), dict()
    fpr_at_tpr95 = dict()
...
def metric(dir_name, stype = 'PoT'):
    tp, fp, fpr_at_tpr95 = get_curve(dir_name, stype=stype)
    results = dict()
    results[stype] = dict()
    # FPR
    mtype = 'FPR'
